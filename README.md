...
8. Verifiy Trivy installation: ``` trivy --version```
...
As soon as the build happens, we need to get an email notification to do that we have to configure our email.
...
1. In the stage 'Tag and Push to DockerHub', give your docker-hub username. 'Deploy to container' stages
...
pipeline {
    ...
    stages {
        ...
        stage ("Git Checkout") {
            steps {
                git 'https://github.com/mohammad-asif-parimoo/zomato.git'
            }
        }
            ...
            steps {
                script {
                    withDockerRegistry(credentialsId: 'docker') {
                        sh "docker tag zomato mohammadasifparimoo/zomato-app:latest"
                        sh "docker push mohammadasifparimoo/zomato-app:latest"
                    }
                }
            }
        }
        stage ("Deploy to Container") {
            steps {
                sh 'docker run -d --name zomato -p 3000:3000 mohammadasifparimoo/zomato-app:latest'
            }
        }
    }
    post {
        always {
            emailext attachLog: true,
                ...
                """,
                to: 'mohammadasifparimoo@gmail.com',
                mimeType: 'text/html',
...
Note 2: Run the VS Code Editor/Power Shell/Command Prompt/Terminal as Administrator, to avoid errors.

Open vs code editor and execute the below commands; 
Step 01: Create EKS Cluster using eksctl
# Create Cluster. I will keep the cluster name as "zomato-cluster"
```
eksctl create cluster --name=zomato-cluster \
                      --region=ap-south-1 \
                      --zones=ap-south-1a,ap-south-1c \
                      --without-nodegroup                  
```
...
Approach 1: Single Line Command

Simply run the entire command in one line without using backslashes for line continuation:

``` eksctl create cluster --name=zomato-cluster --region=ap-south-1 --zones=ap-south-1a,ap-south-1c --without-nodegroup ```


It will take atleast 20-25 minutes for the cluster to create.

To verify the cluster creation ---> Goto Cloud Formation service in AWS ----> You should see a stack got created with the name "zomato-cluster". Make sure in the vs code editor the cluster will get created. As said earlier it will take atleast 20 minutes.
Once the cluster is ready, you will see "EKS Cluster "zomato-cluster" in "us-east-1" region is ready" in vs code editor. wait till you see this.

...
(OR)
```
eksctl utils associate-iam-oidc-provider --region ap-south-1 --cluster zomato-cluster --approve
```
...
```
eksctl create nodegroup --cluster=zomato-cluster \
                       --region=ap-south-1 \
                       --name=zomato-ng-public1 \
                       --node-type=t3.medium \
                       --nodes=2 \
                       --nodes-min=2 \
                       --nodes-max=4 \
                       --node-volume-size=20 \
                       --ssh-access \
                       --ssh-public-key=<Write the PEM key name here> \
                       --managed \
                       --asg-access \
                       --external-dns-access \
                       --full-ecr-access \
...
********************************************************************************
Deploy the same application in the EKS cluster also
...
eksctl get nodegroup --cluster=zomato-cluster
```

# Delete Node Group
```
eksctl delete nodegroup --cluster=<clusterName> --name=<nodegroupName>
eksctl delete nodegroup --cluster=zomato-cluster --name=zomato-ng-public1
```
...
eksctl delete cluster zomato-cluster
```
